###################################################################################################################################################
##
##        Script to automatically Set Maintenance mode OFF for dedicated machines
##
...
{
    $FileExists = Test-Path -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_FailSkip.txt" -PathType Leaf

    if($false -eq $FileExists)
    {
        Add-Content -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_FailSkip.txt" -Value "--Timestamp(UTC)--`tMachine Name`tType`tReason`tController" 
    }

    Add-Content -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_FailSkip.txt" -Value "$($(Get-Date).ToUniversalTime())`t$DCName`t$Type`t$Reason`t$Controller" 
}
...
{
    $FileExists = Test-Path -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_Success.txt" -PathType Leaf

    if($false -eq $FileExists)
    {
        Add-Content -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_Success.txt" -Value "--Timestamp(UTC)--`tMachine Name`tController`tMaintenance Mode" 
    }

    Add-Content -Path "$PSScriptRoot\ServersideLogs\ShortLogs\XD_MM_off_Success.txt" -Value "$($(Get-Date).ToUniversalTime())`t$DCName`t$Controller`t$MaintenanceMode" 
}
...
debug "Successfully retrieved machine data for $ComputerName. Proceeding to set MM OFF..."

Set-BrokerPrivateDesktop -MachineName "$DomainName\$ComputerName" -InMaintenanceMode:$false -AdminAddress $XDController

$XD_Object = Get-BrokerMachine -MachineName "$DomainName\$ComputerName" -AdminAddress $XDController

$MM_OffCheck = $XD_Object.InMaintenanceMode

if($true -eq $MM_OffCheck)
{
    debug "Failed to set Maintenance Mode off for $ComputerName"

    $FailSkipReason = "Failed to set Maintenance Mode Off."
...
debug "Successfully set Maintenance Mode OFF for $ComputerName."

debug "Appending Data to the success file..."

debug_Success -DCName $ComputerName -Controller $XDController -MaintenanceMode $MM_OffCheck
